...
module Discordrb::API::Server
  ...
  def create_channel(token, server_id, name, type, topic, bitrate, user_limit, permission_overwrites, parent_id, nsfw, rate_limit_per_user, position, reason = nil)
    Discordrb::API.request(
      ...
      "/guilds/#{server_id}/channels",
      headers: { Authorization: token, content_type: :json, 'X-Audit-Log-Reason': reason },
      payload: { name: name, type: type, topic: topic, bitrate: bitrate, user_limit: user_limit, permission_overwrites: permission_overwrites, parent_id: parent_id, nsfw: nsfw, rate_limit_per_user: rate_limit_per_user, position: position }
    )
  end
  ...
  def add_emoji(token, server_id, image, name, roles = [], reason = nil)
    Discordrb::API.request(
      ...
      headers: { Authorization: token, content_type: :json, 'X-Audit-Log-Reason': reason },
      payload: { image: image, name: name, roles: roles }
    )
  end
